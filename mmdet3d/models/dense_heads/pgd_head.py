...
class PGDHead(FCOSMono3DHead):
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            #print(pos_bbox_targets_3d[:, -4:].max(-1)[0].max())
            if self.pred_bbox2d:
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list,
        ...
        # expand regress ranges to align with point
        ...
        #pdb.set_trace()
        #(torch.cat(gt_bboxes_list)[:,2:]-torch.cat(gt_bboxes_list)[:,:2]).max()
        #print(torch.cat(bbox_targets_list)[(torch.cat(labels_3d_list)>=0)*(torch.cat(labels_3d_list)<3)].max())
        
        #pos_index = (labels_3d_list[0]>=0)*(labels_3d_list[0]<3)
        #print(bbox_targets_list[0][pos_index].max())
