...
@weighted_loss
def uncertain_l2_loss(pred, target, sigma, alpha=1.0, flood_para=0.0):
    """L1 loss with uncertainty.
    ...
    assert pred.size() == target.size() == sigma.size(), 'The size of pred ' \
        ...
        'are inconsistent.'
    loss = (pred - target)**2
    loss = 0.5* torch.exp(-2*sigma) * loss + alpha * sigma
    return loss
...
class UncertainL1Loss(nn.Module):
    ...
    def __init__(self, alpha=1.0, beta=0.0, reduction='mean', loss_weight=1.0):
...
@LOSSES.register_module()
class UncertainL2Loss(nn.Module):
    """L1 loss with uncertainty.
    ...
    def __init__(self, alpha=1.0, beta=0.0, reduction='mean', loss_weight=1.0):
        super(UncertainL2Loss, self).__init__()
        assert reduction in ['none', 'sum', 'mean']
        self.alpha = alpha
        self.beta = beta
        self.reduction = reduction
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        loss_bbox = torch.exp(self.beta*2*sigma.detach()) * (self.loss_weight)**(-self.beta) * \
            (self.loss_weight * uncertain_l2_loss(
            pred,
            target,
            weight,
            sigma=sigma,
