...
from mmdet3d.core import (CameraInstance3DBoxes, bbox3d2result,
                          show_multi_modality_result,show_multi_modality_result_multilabel)
...
class SingleStageMono3DDetector(SingleStageDetector):
    ...
    def forward_train(self,
        ...
        """
        #self.eval()
        #print('self.eval in single_stage_mono3d.py')
        x = self.extract_feat(img)
        losses = self.bbox_head.forward_train(x, img_metas, gt_bboxes,
                                              gt_labels, gt_bboxes_3d,
                                              gt_labels_3d, centers2d, depths,
                                              attr_labels, gt_bboxes_ignore)
        #losses = {key:losses[key]*0.0 for key in losses.keys()}
        #print('loss set as zero in single_stage_mono3d.py')
        return losses
    ...
    def show_results(self, data, result, out_dir, show=False, score_thr=None):
        ...
        for batch_id in range(len(result)):
            ...
            img = mmcv.imread(img_filename,file_client_args=file_client_args)
            ...
            show_multi_modality_result_multilabel(
                img,
                None,
                pred_bboxes,
                cam2img,
                ...
                file_name,
                'camera',
                label = result[batch_id]['img_bbox']['labels_3d'],
                show=show)
