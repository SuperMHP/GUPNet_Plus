...
def show_multi_modality_result_multilabel(img,
                               gt_bboxes,
                               pred_bboxes,
                               proj_mat,
                               out_dir,
    ...
    '''
    if pred_bboxes is not None:
        if pred_bboxes.tensor.size(1)==9: #nuscenes
            color_tables = ((104,158,48), #car
                            (50,128,246), #truck
                            (202,0,138),  #trailer
                            (10,0,36),    #bus
                            (50,244,253), #construction_vehicle
                            (44,30,216),  #bicycle
                            (138,51,245), #motorcycle
                            (227,76,32),  #pedestrain
                            (255,255,255),#traffic_cone
                            (52,78,142))  #barrier
            thickness = 2
        elif pred_bboxes.tensor.size(1)==7: #kitti 
            color_tables = ((227,76,32),  #pedestrain
                            (44,30,216),  #cyclist
                            (104,158,48)) #car
            thickness = 1
        pred_img = img
        for oid,oid_color in enumerate(color_tables):
            pred_img = draw_bbox(
                pred_bboxes[label==oid], pred_img, proj_mat, img_metas, color=oid_color,thickness=thickness)
