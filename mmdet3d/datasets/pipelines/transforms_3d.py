...
class Color_aug(object):
    ...
    def _blend(self, alpha, image1, image2):
        ...
        image2 *= (1 - alpha)
        image1 += image2
    def _saturation(self, image, gs, gs_mean, var):
        alpha = 1. + self.data_rng.uniform(low=-var, high=var)
        self._blend(alpha, image, gs[:, :, None])
    def _brightness(self, image, gs, gs_mean, var):
        alpha = 1. + self.data_rng.uniform(low=-var, high=var)
        image *= alpha
    def _contrast(self, image, gs, gs_mean, var):
        alpha = 1. + self.data_rng.uniform(low=-var, high=var)
        self._blend(alpha, image, gs_mean)
    def _lighting(self, image, alphastd):
        alpha = self.data_rng.normal(scale=alphastd, size=(3, ))
        image += np.dot(self.eig_vec, self.eig_val * alpha)

    def __call__(self, results):
        img = results['img']/255.
        functions = [brightness_, contrast_, saturation_]
        random.shuffle(functions)
        # ! BGR or RGB may have bugs
        gs = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        gs_mean = gs.mean()
        for f in functions:
            f(image, gs, gs_mean, 0.4)
        self._lighting(image, 0.1)
        results['img'] = img*255.
        return results
            ...
            for key in results.get('bbox_fields', []):
                if key in ['gt_bboxes']:
                    ...
                    if 'attr_labels' in results:
                        results['attr_labels'] = results['attr_labels'][
                            valid_index]
